...
public slots:
    void refresh(void)
    {
        ...
        s += "radius: " + get_distance_str(DISTANCE_FIXED_TO_M(celestial->radius)) + "\n";
        s += "orbital radius: " + get_distance_str(DISTANCE_FIXED_TO_M(celestial->trajectory.orbital_radius));
        if (celestial->trajectory.parent)
        {
            // test v2d shittery
            s += "\ndist from primary: " + get_distance_str(DISTANCE_FIXED_TO_M((celestial->trajectory.position - celestial->trajectory.parent->trajectory.position).length()));
            s += "\nrootfinder error: " + get_distance_str(DISTANCE_FIXED_TO_M((celestial->trajectory.position - celestial->trajectory.parent->trajectory.position).length() - celestial->trajectory.orbital_radius));
            s += "\nNOTE: error should go to 1 or 0 on the racetrack nodes";
        }
        else
        {
            s += "\ndist from primary: " + get_distance_str(DISTANCE_FIXED_TO_M(celestial->trajectory.position.length()));
        }
