...
{
    PyReactorDesignObject *object = (PyReactorDesignObject *)type->tp_alloc(type, 0);
    if (wrapper_newup)
    {
        object->ref = new ReactorDesign();
    }
...
// attribute functions
static PyObject* get_name(PyReactorDesignObject *self, void *closure)
{
    return PyUnicode_FromString(self->ref->name.toStdString().c_str());
}
static int set_name(PyReactorDesignObject *self, PyObject *value, void *closure)
{
    if (value == NULL)
    {
...
static PyObject* get_uses_power(PyReactorDesignObject *self, void *closure)
{
    return PyBool_FromLong(self->ref->uses_power);
}
static PyObject* get_produces_power(PyReactorDesignObject *self, void *closure)
{
    return PyBool_FromLong(self->ref->produces_power);
}
static PyObject* get_circuit(PyReactorDesignObject *self, void *closure)
{
    if (!self->ref->circuit)
        Py_RETURN_NONE;
    return (PyObject*)pyobjectize_circuitdesign(self->ref->circuit);
}
static int set_circuit(PyReactorDesignObject *self, PyObject *value, void *closure)
{
    if (value == NULL)
    {
        PyErr_SetString(PyExc_TypeError, "Cannot delete attribute.");
    ...
    {
        PyErr_SetString(PyExc_TypeError, "Can only assign CircuitDesign type to ReactorDesign.circuit.");
        return -1;
    }
...
static PyObject *func_descriptor_string(PyReactorDesignObject *self, PyObject *args)
{
...
static PyObject* __eq__(PyReactorDesignObject *self, PyObject *other, int op)
{
    if (!PyObject_IsInstance(other, (PyObject *)&PyReactorDesignType))
        Py_RETURN_FALSE;
    PyReactorDesignObject *other_cast = (PyReactorDesignObject *)other;
    if (op == Py_EQ && self->ref == other_cast->ref)
        Py_RETURN_TRUE;
    Py_RETURN_FALSE;
}

PyTypeObject PyReactorDesignType = {
    PyVarObject_HEAD_INIT(NULL, 0)
    .tp_name = "paxpython.ReactorDesign",
    .tp_basicsize = sizeof(PyReactorDesignObject),
    .tp_itemsize = 0,
    .tp_dealloc = (destructor)type_dealloc,
    .tp_flags = Py_TPFLAGS_DEFAULT,
    .tp_doc = PyDoc_STR("PaxPython ReactorDesign Type Wrapper."),
...
{
    wrapper_newup = false;
    PyReactorDesignObject *pyobj = (PyReactorDesignObject *)PyObject_Call((PyObject *)&PyReactorDesignType,PyTuple_New(0),NULL);
    pyobj->ref = obj;
    pyobj->tracked = true;
    wrapper_newup = true;
    return pyobj;
...
{
    if (PyType_Ready(&PyReactorDesignType) < 0)
    {
        Py_DECREF(m);
        return false;
    }
    Py_INCREF(&PyReactorDesignType);
    if (PyModule_AddObject(m, "ReactorDesign", (PyObject *)&PyReactorDesignType) < 0)
    {
        Py_DECREF(&PyReactorDesignType);
        Py_DECREF(m);
        return false;
    }
    return true;
